...
# URA Taxpayer Identification Number (10 digits starting with 1)
_TIN_EXPLICIT_RE = re.compile(
    r"\bTIN(?::|\s+is|\s+number|\s+no\.?)?\s*([1-9]\d{9})\b", re.IGNORECASE
)
...
# Acronyms in Ugandan tax context
_ACRONYMS = [
    ...
    (re.compile(r"\bDTS\b"), "Digital Tracking Solutions"),
...
def clean_text_for_speech(text: str, locale: str = "en") -> str:
    ...
    # 7. Normalize 10-digit TINs (spell out digits clearly)
