...
class MetadataIsNotAFigureTests(unittest.TestCase):
    ...
    # The companion case — that quoting a *provisional* figure still fails
    # closed — moved to ProvisionalTableTests, which builds a synthetic
    # provisional year. Every shipped table is confirmed since FY2026-27 was
    # reconciled against the Act, so pinning that behaviour to a real year
    # would assert the reconciliation had not happened.
