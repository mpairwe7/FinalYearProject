...
def predict_tag(question: str, context: str = "") -> dict:
    ...
    if clf is None:
        return {"error": "Model not loaded", "tag": None, "confidence": 0}
    ...
    try:
        ...
        top_tags = [
            {"tag": encoder.classes_[i], "confidence": float(proba[i])} for i in top_indices
        ]

        return {"tag": tag, "confidence": confidence, "top_predictions": top_tags}
    except Exception as e:
        return {"error": str(e), "tag": None, "confidence": 0}
...
with gr.Blocks(title="URA Chatbot - Tag Classifier") as demo:
    ...
    with gr.Row():
        with gr.Column():
            question_input = gr.Textbox(
                label="Question", placeholder="e.g., How do I pay VAT online?", lines=2
            )
    ...
    submit_btn.click(fn=gradio_predict, inputs=[question_input, context_input], outputs=output)
