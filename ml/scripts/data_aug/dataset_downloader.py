...
def download_all_corpora(
    ...
    include_waxal: bool = True,
    include_flores_eval: bool = True,
) -> list[Path]:
    """Download all available corpora for Ugandan languages.

    Sources (by priority):
    1. Sunbird SALT — 25k parallel (nyn/ach/lg) + ASR + TTS (CC-BY-SA-4.0)
    2. Google WAXAL — 132k nyn + 114k ach ASR/TTS (CC-BY-SA-4.0)
    3. JW300 — parallel MT (CC0)
    4. OPUS Tatoeba — short parallel sentences
    5. Masakhane LAFAND-MT — African MT benchmark (CC-BY-NC-4.0)
    6. FLORES-200 — MT eval benchmark (CC-BY-SA-4.0)
    7. Google FLEURS — ASR eval (CC-BY-4.0)
    """
    ...
    for src, tgt in lang_pairs:
        ...
        # OPUS Mozilla-I10n has 24k Acholi pairs (Firefox localization).
        if tgt == "ach" or src == "ach":
            try:
                paths.append(download_opus("Mozilla-I10n", src, tgt, output_dir / "opus"))
            except Exception as e:
                log.warning("Mozilla-I10n %s-%s unavailable: %s", src, tgt, e)
...
def main():
    ...
    parser.add_argument("--no-salt", action="store_true", help="Skip Sunbird SALT download")
    parser.add_argument("--no-waxal", action="store_true", help="Skip Google WAXAL download")
    ...
    parser.add_argument("--no-flores", action="store_true", help="Skip FLORES-200 download")
