    ...
    class _Active:
        ...
        def log_params(self, d):
            with contextlib.suppress(Exception):
                self.m.log_params({k: str(v)[:500] for k, v in d.items()})

        def log_metrics(self, d, step=None):
            for k, v in d.items():
                if isinstance(v, int | float):
                    with contextlib.suppress(Exception):
                        self.m.log_metric(k, float(v), step=step)

        def log_artifact(self, path):
            with contextlib.suppress(Exception):
                self.m.log_artifact(str(path))
...
def load_qa_data(datasets_dir: Path, config: dict) -> pd.DataFrame:
    ...
    for path in csv_files:
        try:
            ...
            q_col = next(
                (columns_lower[c] for c in columns_lower if c in question_candidates), df.columns[0]
            )
            a_col = next(
                (
                    columns_lower[c]
                    for c in columns_lower
                    if c in answer_candidates and columns_lower[c] != q_col
                ),
                df.columns[-1],
            )
...
def train_classifier(X: np.ndarray, y: np.ndarray, config: dict) -> tuple[Any, dict[str, float]]:
    ...
    skf = StratifiedKFold(
        n_splits=n_splits, shuffle=True, random_state=config["data"]["random_seed"]
    )
        ...
        cv_metrics.append(
            {
                ...
                "precision": precision_score(
                    y[val_idx], val_pred, average="macro", zero_division=0
                ),
                "recall": recall_score(y[val_idx], val_pred, average="macro", zero_division=0),
            }
        )
        print(
            f"  Fold {fold}: Acc={cv_metrics[-1]['accuracy']:.4f}, F1={cv_metrics[-1]['f1_macro']:.4f}"
        )
...
def export_models(
    clf: Any, label_encoder: LabelEncoder, X_sample: np.ndarray, output_dir: Path, config: dict
) -> dict[str, str]:
