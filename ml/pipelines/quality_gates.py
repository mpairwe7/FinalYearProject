...
def _prod_check_per_language(results: dict, rag_eval: dict | None, gates: dict) -> None:
    ...
    for lang, metrics in slices.items():
        ...
        if not floor:
            continue
        for metric_name in (
            'faithfulness', 'numerical_accuracy', 'law_citation_accuracy',
        ):
...
def _print_results(family: str, results: dict) -> None:
    ...
    for check in results['checks']:
        ...
        if isinstance(value, float):
            value_s = f"{value:>8.4f}"
        else:
            value_s = f"{str(value):>8}"
        threshold = check['threshold']
        if isinstance(threshold, float):
            thresh_s = f"{threshold:<8.4f}"
        else:
            thresh_s = f"{str(threshold):<8}"
        print(
            f"  {check['name']:40} {value_s} {comparator} {thresh_s}  {status}"
        )
    print("-" * 70)
...
def main():
    ...
    prod.add_argument('--rag-eval', type=Path, default=None,
                      help='Path to rag_evaluation_results.json')
    prod.add_argument('--safety', type=Path, default=None,
                      help='Path to safety_evaluation_results.json')
    prod.add_argument('--calibration', type=Path, default=None,
                      help='Path to calibration_report.json')
    prod.add_argument('--tokenizer-audit', type=Path, default=None,
                      help='Path to tokenizer_audit.json')
    prod.add_argument('--benchmark', type=Path, default=None,
                      help='Path to benchmark.json')
    prod.add_argument('--mobile-manifest', type=Path, default=None,
                      help='Path to mobile_manifest.json')
    prod.add_argument('--model-card', type=Path, default=None,
                      help='Path to MODEL_CARD.md')
