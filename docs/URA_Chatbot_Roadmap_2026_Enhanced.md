...
| G31 🟡 | No admin UI | Partial | **Shipped 2026-08-18:** `/admin/flags` + `GET/PATCH /v1/admin/flags` persist to replica-local `flag_overrides` (safety flags protected). Still open: content curation, Git-backed prompt editor, GrowthBook, cluster-wide control plane. | `frontend/src/app/admin/flags/`, `backend/app/flags.py` | L |
