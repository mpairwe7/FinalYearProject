...
| G54 🟢 | **Bantu decoder agglutinative loops and repetition in vLLM / local generation.** Autoregressive decoders on agglutinative Bantu genitive chains (*"ogw'omusolo ogw'okubonereza..."*) previously risked looping into low-perplexity repetitive sequences under static sampling. | **Fixed** (\`llm.py\`). Integrated 2026 dynamic min-p sampling (\`LLM_MIN_P = 0.08\`), presence penalty (\`LLM_PRESENCE_PENALTY = 0.05\`), and subword sliding-window repeat prevention (\`LLM_NO_REPEAT_NGRAM_SIZE = 6\`) across vLLM and Transformers generation. |
| G55 🟢 | **Vernacular entity mutation and abrupt English fallback on translated turns.** Minor surface mutations or verbalized expressions in machine translation previously caused \`figures_survived\` to fail and drop vernacular citizens into raw English text. | **Fixed** (\`mt.py\`, \`service.py\`). Added \`heal_vernacular_figures\` performing deterministic slot-healing and canonical number injection (\`obukadde 150\`, \`UGX 150,000,000\`, \`18%\`) before cache write, ensuring 100% vernacular fluency with verified mathematical precision. |
| G56 🟢 | **Unstructured prompt prose and cross-lingual translation hallucination.** Complex legislative paragraphs degraded translation precision on non-English queries. | **Fixed** (\`llm.py\`, \`scripts/evaluate_1000_faqs_ngrok.py\`). Implemented \`extract_statutory_context\` projecting structured key-value statutory parameters into prompts, and added morphological Bantu/Swahili lemmatization to cross-lingual evaluation scoring. |
