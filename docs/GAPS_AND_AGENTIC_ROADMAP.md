...
| G56 🟡 | **Unstructured prompt prose and cross-lingual translation hallucination.** Complex legislative paragraphs degraded translation precision on non-English queries. | **Partly fixed** (`llm.py`, `scripts/evaluate_1000_faqs_ngrok.py`). `extract_statutory_context` projects structured key-value statutory parameters into prompts, and cross-lingual evaluation scoring gained Bantu/Swahili prefix lemmatization. **Open, tracked for a follow-up:** (a) `extract_statutory_context` re-reads `p["text"]` raw, after `scan_retrieved_text` has already scrubbed and trimmed the same passages in `_build_messages`, so a poisoned or budget-trimmed passage's numbers reach the prompt through a privileged `## Statutory Parameters` header without the LLM01 scrub the module docstring promises; (b) slots carry no entity label and no passage marker, so several rates in context arrive as an unattributed menu; (c) `slots[:5]` truncates in passage order rather than by rank; (d) the eval stemmer was loosened in the same series that reports accuracy gains, so those scores are not comparable to the pre-#478 baseline until it is re-run under the new scorer. |
